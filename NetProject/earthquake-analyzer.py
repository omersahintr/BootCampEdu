...
def response_url():
    ...
    dict = {}
    url = f"https://servisnet.afad.gov.tr/apigateway/deprem/apiv2/event/filter?start={start_date_year}-{start_date_month}-{start_date_day}%20{start_date_hour}:{start_date_minute}:{start_date_second}&end={end_date_year}-{end_date_month}-{end_date_day}%20{end_date_hour}:{end_date_minute}:{end_date_second}" #for afad live data
    ...
    if response.status_code == 200:   
        for query in response.json():
            if query["province"] == locator and float(query["magnitude"])>mag:
                i+=1
                plt.scatter((query["date"]), float(query["magnitude"]), color="red")
